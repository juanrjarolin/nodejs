...
async function crearTarea(req, res) {
  ...
  if (!jsonBody  || !jsonBody.description) {
    res.statusCode = 400;
    res.send({
      message: `campo requerido no encontrado`,
      details: "This endpoint expected JSON object with attribute `description`"
    });
    res.end();
    return;
...
async function editarTarea(req, res) {
  ...
  try{
    const tarea = await tareasLogic.getOne(tareaId);
  }
  catch(error){
    res.statusCode = 400;
    res.send({
      message: `Can't find objecti with id : ${tareaId}`,
      details: "This endpoint expected a valid object identifier"
    });
    res.end();
  }
  ...
  if (!jsonBody  || !jsonBody.estado) {
    res.statusCode = 400;
    res.send({
      message: `campo requerido no encontrado`,
      details: "This endpoint expected JSON object with attribute `estado`"
    });
    res.end();
    return;
  }else if(jsonBody.estado == "TERMINADO" || jsonBody.estado == "PENDIENTE" || jsonBody.estado == "CANCELADO" ){
    const tareaEditada = await tareasLogic.update(tareaId, jsonBody.estado);
    res.send(tareaEditada);
    res.end();
  }else{
    res.send({
      message: `Estado no valido`,
      details: "Estado no valido ,Los estados validos son : `TERMINADO`,`PENDIENTE` y `CANCELADO`"
    });
    res.end();
  }
}
...
async function borrarTarea(req, res) {
  ...
  try{
    await tareasLogic.erase(tareaId);
    res.end("Borrado la tarea : " + req.params.idTarea);
  }
  catch(error){
    res.statusCode = 400;
    res.send({
